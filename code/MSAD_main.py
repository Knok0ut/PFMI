...
import argparse
from util import MSAD as mutils
from tqdm import tqdm
import torch.nn.functional as F
from util.datasets import load_dataset
from util.models import load_model, get_architecture
...
def run_epoch(model, train_loader, optimizer, center, device, is_angular):
    ...
    for ((img1, img2), _) in tqdm(train_loader, desc='Train...'):
        ...
        # 计算对比损失时再做差
        # loss = contrastive_loss(out_1 - center, out_2 - center)
...
def get_score(model, device, train_loader, test_loader):
    ...
    distances = mutils.knn_score(train_feature_space, test_feature_space)
...
def main(args):
    ...
    print(device)
    model = get_architecture(f"{args.backbone}_{args.dataset}")
    # model = mutils.Model(args.backbone)
    model = model.to(device)
    model, _ = load_model(model, f"../checkpoints/{args.dataset}_{args.backbone}_shadow.pt")
    model = mutils.freeze_parameters(model, args.backbone)

    train_loader, test_loader, train_loader_1 = mutils.get_loaders(dataset=args.dataset, label_class=args.label,
                                                                   batch_size=args.batch_size, backbone=args.backbone)

    model = train_model(model, train_loader, test_loader, train_loader_1, device, args)
    if not os.path.exists("../checkpoints/MSAD/"):
        os.mkdir("../checkpoints/MSAD/")
    torch.save(model.state_dict(), f"../checkpoints/MSAD/{args.dataset}_{args.backbone}_{args.label}.pt")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('--dataset', default='cifar10')
    parser.add_argument('--epochs', default=200, type=int, metavar='epochs', help='number of epochs')
